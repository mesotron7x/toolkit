...
- Configures Git defaults in `%USERPROFILE%\.gitconfig` with Mesotron7x identity, color output, Vim as the editor, disabled Git pagers, `main` as the default initial branch, and fast-forward-only pulls/merges, while preserving unrelated existing Git settings.
